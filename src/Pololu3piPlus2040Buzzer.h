...
/// \file Pololu3piPlus2040Buzzer.h
...
/// \brief Plays beeps and music on the buzzer on the 3pi+ 2040.
///
/// This class uses Timer 4 and pin 7 to play beeps and melodies on
/// the 3pi+ 2040 buzzer.
///
...
class Buzzer
{
  ...
  volatile unsigned int buzzerTimeout = 0;    // tracks buzzer time limit
  volatile PlayMode play_mode_setting = PLAY_AUTOMATIC;

  // music settings and defaults
  volatile unsigned char octave = 4;                 // the current octave
  volatile unsigned int whole_note_duration = 2000;  // the duration of a whole note
  volatile unsigned int note_type = 4;               // 4 for quarter, etc
  volatile unsigned int duration = 500;              // the duration of a note in ms
  volatile unsigned int volume = 15;                 // the note volume
  volatile unsigned char staccato = 0;               // true if playing staccato

  // staccato handling
  volatile unsigned char staccato_rest_duration;  // duration of a staccato rest,
